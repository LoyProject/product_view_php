<?php
    ...
    $sql = "SELECT name, contact, address, email, description, location, facebook, telegram, youtube, logo_header, logo_footer, image1, image2, image3, image4, slideshow_images FROM companies WHERE id = 1";
    $stmt = $conn->prepare($sql);
    $stmt->execute();
    $stmt->bind_result($name, $contact, $address, $email, $description, $location, $facebook, $telegram, $youtube, $logoHeader, $logoFooter, $image1, $image2, $image3, $image4, $slideshow_images_json);
    $stmt->fetch();
    $stmt->close();
...
<body>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if ("<?php echo $image4; ?>") {
            document.getElementById('preview-image4').src = "../images_logo/<?php echo $image4; ?>";
            document.getElementById('preview-image4').classList.remove('hidden');
            document.getElementById('file-name4').style.display = 'none';
            document.getElementById('upload-icon4').style.display = 'none';
        }
    ...
    function resetImagePreview() {
        ...
        const fileNameLabel4 = document.getElementById('file-name4');
        const uploadIcon4 = document.getElementById('upload-icon4');
        ...
        previewImage4.classList.add('hidden');
        previewImage4.value = '';
        fileNameLabel4.style.display = 'block';
        uploadIcon4.style.display = 'block';
                ...
                <div class="container-xl mx-auto">
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <a href="new_product.php">
                            <button
                                class="hidden bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-700">
                                Add New
                            </button>
                        </a>
                    </div>
                    ...
                    <div class="container mx-auto">
                        <form id="editCompanyForm" onsubmit="editCompany(event)">
                            ...
                            <div class="mb-6">
                                <label for="slideshow-images" class="block text-gray-700 font-medium mb-2">Slideshow
                                    Images</label>
                                <label for="slideshow-images"
                                    ...
                                    <span id="file-name-slideshow" class="text-gray-400 text-center">
                                        <strong>Upload Multiple Images</strong><br>Only .png, .jpeg, .jpg files are
                                        allowed.
                                    </span>
                                    <input type="file" id="slideshow-images" name="slideshow_images[]"
                                        accept=".jpg, .jpeg, .png" multiple class="hidden"
                                        onchange="previewSlideshowImages(event);">
                                    <div id="slideshow-preview"
                                        class="hidden grid grid-cols-3 gap-4 max-h-48 overflow-y-auto p-2"></div>
                                </label>
                            </div>

                            <div class="mb-6">
                                <div class="block text-gray-700 font-medium mb-2">Upload Images (Max 4)</div>
                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    <div class="border-2 border-dashed border-gray-300 shadow-sm rounded cursor-pointer flex flex-col justify-center items-center h-32"
                                        ...
                                        <input type="file" id="image1" name="image1" accept=".jpg, .jpeg, .png"
                                            class="hidden"
                                            onchange="displayFileName1(this); document.getElementById('file-name1').style.display = 'none'; document.getElementById('upload-icon1').style.display = 'none';">
                                    ...
                                    <div class="border-2 border-dashed border-gray-300 shadow-sm rounded cursor-pointer flex flex-col justify-center items-center h-32"
                                        ...
                                        <input type="file" id="image2" name="image2" accept=".jpg, .jpeg, .png"
                                            class="hidden"
                                            onchange="displayFileName2(this); document.getElementById('file-name2').style.display = 'none'; document.getElementById('upload-icon2').style.display = 'none';">
                                    ...
                                    <div class="border-2 border-dashed border-gray-300 shadow-sm rounded cursor-pointer flex flex-col justify-center items-center h-32"
                                        ...
                                        <input type="file" id="image3" name="image3" accept=".jpg, .jpeg, .png"
                                            class="hidden"
                                            onchange="displayFileName3(this); document.getElementById('file-name3').style.display = 'none'; document.getElementById('upload-icon3').style.display = 'none';">
                                        <img id="preview-image3" class="hidden px-2 w-full h-28 object-contain rounded">
                                    </div>
                                    <div class="border-2 border-dashed border-gray-300 shadow-sm rounded cursor-pointer flex flex-col justify-center items-center h-32"
                                        onclick="document.getElementById('image4').click()">
                                        <div id="upload-icon4" class="text-gray-500 mb-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-11" viewBox="0 0 32 32">
                                                <path
                                                    d="M23.75 11.044a7.99 7.99 0 0 0-15.5-.009A8 8 0 0 0 9 27h3a1 1 0 0 0 0-2H9a6 6 0 0 1-.035-12 1.038 1.038 0 0 0 1.1-.854 5.991 5.991 0 0 1 11.862 0A1.08 1.08 0 0 0 23 13a6 6 0 0 1 0 12h-3a1 1 0 0 0 0 2h3a8 8 0 0 0 .75-15.956z"
                                                    fill="currentColor" />
                                        ...
                                        <span id="file-name4" class="text-gray-400 text-center">
                                            <strong>Upload Image 3</strong><br>Only .png, .jpeg, .jpg files are allowed.
                                        </span>
                                        <input type="file" id="image4" name="image4" accept=".jpg, .jpeg, .png"
                                            class="hidden"
                                            onchange="displayFileName3(this); document.getElementById('file-name4').style.display = 'none'; document.getElementById('upload-icon3').style.display = 'none';">
                                        <img id="preview-image4" class="hidden px-2 w-full h-28 object-contain rounded">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-6">
                                <label for="image-header" class="block text-gray-700 font-medium mb-2">Logo
                                    Header</label>
                                <label for="image-header"
                                    ...
                                    <input type="file" id="image-header" name="image-header" accept=".jpg, .jpeg, .png"
                                        class="hidden"
                                        onchange="displayFileNameHeader(this); document.getElementById('file-name-header').style.display = 'none'; document.getElementById('upload-icon-header').style.display = 'none';">
                                    <img id="preview-image-header"
                                        class="hidden px-2 w-full h-48 object-contain rounded">
                                </label>
                            </div>

                            <div class="mb-6">
                                <label for="image-footer" class="block text-gray-700 font-medium mb-2">Logo
                                    Footer</label>
                                <label for="image-footer"
                                    ...
                                    <input type="file" id="image-footer" name="image-footer" accept=".jpg, .jpeg, .png"
                                        class="hidden"
                                        onchange="displayFileNameFooter(this); document.getElementById('file-name-footer').style.display = 'none'; document.getElementById('upload-icon-footer').style.display = 'none';">
                                    <img id="preview-image-footer"
                                        class="hidden px-2 w-full h-48 object-contain rounded">
                                </label>
                            </div>
