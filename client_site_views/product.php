...
<?php
    ...
    if (!empty($categoryFilter)) {
        $query .= " AND products.category_id = '$categoryFilter'";
    }
...
<body class="bg-[#f8f9ff]">
    <div class="px-8 mb-8 mt-24 container-md mx-auto flex">
        ...
        ?>
        <div class="w-full md:w-1/5 pr-6 mb-6 md:mb-0">
            <div class="bg-white rounded-lg h-full p-4">
                <h1 class="p-4 text-2xl font-bold">Categories</h1>
                <div class="bg-gray-50 rounded-lg">
                    <ul class="py-4 px-2">
                        <li class="px-2 py-2">
                            <a href="product.php"
                                class="block py-2 px-6 font-bold rounded-lg hover:bg-gray-200 <?= empty($categoryFilter) ? 'text-white bg-red-500 hover:bg-red-500' : 'text-gray-700' ?>">
                                All
                            </a>
                        </li>
                        ...
                        <li class="px-2 py-2">
                            <a href="product.php?category=<?= $cat['id'] ?>"
                                class="block py-2 px-6 font-bold rounded-lg hover:bg-gray-200 <?= $categoryFilter == $cat['id'] ? 'text-white bg-red-500 hover:bg-red-500' : 'text-gray-700' ?>">
        ...
        <div class=" w-4/5 pl-6 p-4 bg-white rounded-lg">
            <h1 class="text-2xl font-bold mb-4">Products</h1>
            <form method="GET" class="flex gap-4 mb-6">
                <input type="text" name="search" value="<?= htmlspecialchars($search) ?>"
                    placeholder="Search by name, category, description"
                    class="w-full p-4 border-2 border-red-500 rounded-md">
                <button type="submit" class="w-24 text-center bg-red-500 text-white p-4 rounded-md">Search</button>
                <a href="product.php" class="w-24 text-center bg-gray-500 text-white p-4 rounded-md">Clear</a>
            </form>

            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <?php while ($product = $result->fetch_assoc()): ?>
                <div
                    class="bg-white shadow-[0_4px_12px_-5px_rgba(0,0,0,0.4)] border p-2 w-full rounded-lg font-[sans-serif] overflow-hidden mx-auto mt-4 hover:border-red-500 border-2 flex flex-col">
                    <div>
                        <img src="../images/<?= htmlspecialchars($product['image']) ?>"
                            class="w-full h-52 object-cover rounded-lg" />
                    </div>
                    <div class="pt-6 text-start px-2">
                        <h3 class="text-lg font-bold font-[sans-serif]">
                            <?= htmlspecialchars($product['name']) ?></h3>
                    </div>
                    <div class="text-start flex-grow px-2">
                        <p class="mt-1 text-xs text-red-500 font-bold tracking-wider font-[sans-serif]">
                            <?= htmlspecialchars($product['category_name']) ?></p>
                    </div>
                    <div class="text-start flex-grow px-2">
                        <article class="text-wrap">
                            <p class="mt-3 text-xs text-gray-500 leading-relaxed break-words">
                                <?= htmlspecialchars($product['description']) ?></p>
                        </article>
                    </div>
                    <div class="p-2 mt-4">
                        <a href="product_detail.php?id=<?= htmlspecialchars($product['id']) ?>"
                            class="w-full py-2.5 inline-block text-center rounded-sm bg-red-100 text-red-500 text-xs font-bold tracking-wider font-[sans-serif] outline-none hover:bg-red-400 hover:text-white">
                            View
                        </a>
                    </div>
                </div>
